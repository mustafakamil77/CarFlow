...
class Branch(models.Model):
    ...
    def ensure_qr_token(self):
        ...
        while True:
            ...
            if len(token) < 32:
                continue
            if not Branch.objects.filter(qr_token=token).exists():
                self.qr_token = token
                return
    ...
    def generate_qr_code_image(self, *, force=False):
        ...
        label = slugify(self.legal_name or self.name or "branch") or "branch"
        token_prefix = (self.qr_token or "")[:8]
        name = f"qr_branch_{label}_{self.pk or 'new'}_{token_prefix}.png"
        self.qr_code_image.save(name, ContentFile(buffer.getvalue()), save=False)
...
class BranchDocument(models.Model):
    ...
    file = models.FileField(upload_to="branch_documents/", null=True, blank=True)
...
class CarEventAttachment(models.Model):
    event = models.ForeignKey(
        CarEvent,
        on_delete=models.CASCADE,
